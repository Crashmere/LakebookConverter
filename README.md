...
默认情况下，图片下载成功时会在文档输出目录下创建 `attachments/` 子目录；使用 `--nopic` 时不会创建。下载图片时会自动重试，若最终仍失败，或缺少图片处理依赖，则在原文位置写入缺失提示与原始 URL 文本：
...
| `beautifulsoup4` | 推荐 | HTML 解析（LaTeX 提取、图片下载） | 跳过 LaTeX 还原；图片改为缺失提示与原始 URL 文本 |
| `requests` | 可选 | 默认下载外链图片（使用 `--nopic` 可跳过） | 图片无法下载；Markdown 中写入缺失提示与原始 URL 文本 |
...
**Q：为什么文档里会出现“图片缺失”提示和 URL 文本？**

A：工具会自动重试图片下载；如果多次重试后仍失败，或缺少 `beautifulsoup4` / `requests` 这类图片处理依赖，则不会再保留图片外链，而是把该位置替换为缺失提示和原始 URL 文本，提醒你后续手动补齐。命令行输出中也会给出警告和成功/失败统计。
